...
BoolResult IntegerType::isExplicitlyConvertibleTo(Type const& _convertTo) const
{
	...
	{
		// TODO Decide on this.
		if (!isSigned() || numBits() == enumType->storageBytes() * 8)
			return true;
	}
	else if (dynamic_cast<ContractType const*>(&_convertTo))
	{
		// TODO decide on this. Also Integer -> ContractType
		if (!isSigned() || numBits() == 160)
			return true;
	}
	else if (_convertTo.category() == Category::FixedPoint)
...
BoolResult EnumType::isExplicitlyConvertibleTo(Type const& _convertTo) const
{
	...
	{
		// TODO decide on the conversion here
		if (!integerType->isSigned())
			return true;
		else if (integerType->numBits() != storageBytes() * 8)
			return true;
	}
	return false;
}
